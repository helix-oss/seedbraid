...
def test_publish_warns_for_unencrypted_seed(tmp_path: Path, monkeypatch) -> None:
...
def test_publish_skips_warning_for_encrypted_seed(tmp_path: Path, monkeypatch) -> None:
    seed = tmp_path / "encrypted.hlx"
    seed.write_bytes(b"HLE1" + b"x" * 64)
    monkeypatch.setattr("helix.cli.publish_seed", lambda _seed, pin=False: "bafyencrypted")
